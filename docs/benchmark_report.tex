...
\begin{itemize}[leftmargin=1.4em, itemsep=2pt]
  \item \textbf{Frontend:} Static HTML/CSS/JS on Vercel
...
The system classifies images into eight active categories.
Four are designated \highriskbadge{high-risk}; a positive high-risk prediction
...
SCC  & Squamous cell carcinoma     &  0.8\% & \highriskbadge{High} \\
\bottomrule
...
\begin{enumerate}[leftmargin=1.6em, itemsep=4pt]
  \item \textbf{Class-rebalanced fine-tuning} (in development) ---
        Square-root frequency sampling combined with LDAM loss
        \cite{cao2019learning} to address the SCC/AK/DF recall deficit.
        Based on published ISIC ablations, expected improvement is
        $+15$--$25\%$ recall on minority classes with minimal regression
        on common classes.

  \item \textbf{Asymmetric loss} \cite{ridnik2021asl} ---
        Decoupled positive/negative focusing ($\gamma_- > \gamma_+$)
        to suppress NV gradient dominance without requiring explicit
        per-class weight tuning.

  \item \textbf{EVA-02 backbone} (planned) ---
        EVA-02-Base \cite{fang2023eva} (86\,M parameters, Apache\,2.0,
        available via \code{timm}) was a top-performing architecture in
        ISIC\,2024 competition solutions, outperforming EfficientNet
        ensembles by 5--12\% AUC in published solution write-ups.

  \item \textbf{GPU inference} (deployed) ---
        Modal backend upgraded to NVIDIA T4, achieving 8--15$\times$
        inference speedup versus the prior CPU-only deployment.

  \item \textbf{Temperature calibration} (partial) ---
        Per-class temperatures are currently heuristic. Formal calibration
        via cross-entropy minimisation on a held-out split is planned.
\end{enumerate}
